...
def plot_time_series(df, selected_stocks, y_axis):
    fig = go.Figure()
    percent_changes = {}
...
def main():
    ...
    with st.sidebar:
        ...
        selection_mode = st.radio(
            "Selection Mode",
            ('Manual Selection', 'Top 10 Stocks by Change')
        )
        ...
        if selection_mode == 'Manual Selection':
            ...
            # Fetch data for the selected period and calculate changes
            ...
            # Select top 10 stocks by change
            selected_stocks = sorted(changes, key=changes.get, reverse=True)[:10]

    if selected_stocks:
        interval = '1m' if period == '1d' else '1d'
        if period == 'Custom Date Range':
            timeframe_data = get_timeframe_data(selected_stocks, start_date=start_date, end_date=end_date, interval=interval)
        else:
            timeframe_data = get_timeframe_data(selected_stocks, period=period, interval=interval)
        plot_time_series(timeframe_data, selected_stocks, y_axis)
    else:
